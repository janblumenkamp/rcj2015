...
const int16_t STEER_INNER_IMPASSE = 400; //Beim Finden von Sackgasse so in Sackgasse drehen, danach entsprechend kurz in andere Richtung drehen
...
const uint16_t MRC_PAST_WALL_R = 265;    // So weit soll der Roboter weiterfahren, wenn eine Außenkurve dedektiert wird (weil Sensor schräg ist, erkennt er die Kurven eher und muss daher später abbigen)
...
////////////////////////////////////////////////////////////////////////////////
	...
	wdt_enable(WDTO_500MS); //Watchdog aktivieren (funktioniert ab 120MS, Spielraum lassen)
...
///////////////////////Rampe////////////////////////////////////////////////////
		...
		if((accl_median > threedbs_var) || (accl_median < 100))// || (frontsensor_v_raw < 50))
		{
			rampe = 1;
			setLEDs(0b1111);
		}
		if((wallsensor_v_r > WALLSENSOR_MINWALL) || (accl_median < threedbs_var)) //Rampe zu ENde
		{
			rampe = 0;
			setLEDs(0b0000);
		}
		...
		{
			//Rampe
			if(iSteer < -STEER_RAMP_MAX)
			{
				iSteer = -STEER_RAMP_MAX;
			}
			else if(iSteer > STEER_RAMP_MAX)
			{
				iSteer = STEER_RAMP_MAX;
			}
		}
...
////////////////////////////Debugging///////////////////////////////////////////
		...
		if(timer_lcd_refresh == 0)
		{
			...
			setCursorPosLCD(1, 7);
			writeIntegerLCD(accl_median, 10);
			/*setCursorPosLCD(0, 10);
			writeIntegerLCD(mlx90614_r, 10);
